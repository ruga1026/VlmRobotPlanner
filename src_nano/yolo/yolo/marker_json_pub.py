...
class YoloMapMarkerMulti(Node):
    def __init__(self):
        ...
        self.declare_parameter("look_center_frac", 0.50)
        ...
        # JSON 토픽 퍼블리시 관련
        self.declare_parameter("json_topic", "/detected_objects_json")
        self.declare_parameter("publish_json_every_tick", True)
        ...
        self.publish_json_every_tick = bool(self.get_parameter("publish_json_every_tick").value)

        # 퍼블리셔 생성 (이미 std_msgs.msg.String import 되어 있음)
    ...
    # 2) 프롬프트가 있을 때만 YOLOE 클래스 재설정
        ...
        # 🔒 모델 헤드 재세팅 직후 한 프레임 쉬어 shape 에러 방지
            self.skip_one_frame = True
            self.get_logger().info(f"Prompts set (append mode; no reset): {', '.join(self.prompts)}")
        
        else:
            self.get_logger().info("Prompts paused (no reset; keeping JSON/markers/tracks).")
    ...
    def _write_json_if_dirty(self):
        ...
        try:
            ...
            data = [{"objects": self._objects_snapshot()}]   # <- 여기만 교체
    ...
    def _objects_snapshot(self):
        # active_order 순서를 보존해 현재 활성 객체 리스트를 스냅샷으로 구성
        return [self.active_map[i] for i in self.active_order if i in self.active_map]

    def _publish_json(self):
        try:
            payload = [{"objects": self._objects_snapshot()}]
            msg = String()
            msg.data = json.dumps(payload, ensure_ascii=False)
            self.json_pub.publish(msg)
        except Exception as e:
            self.get_logger().warn(f"JSON publish 실패: {e}")
    ...
    def timer_cb(self):
        # 매 틱 말미에 json을 기록
        def finish():
            # 매 틱 마지막에 JSON 퍼블리시(항상) + 파일 저장(변경시만)
            if self.publish_json_every_tick:
                self._publish_json()
            self._write_json_if_dirty()
        if getattr(self, "skip_one_frame", False):
            self.skip_one_frame = False
            return finish()
