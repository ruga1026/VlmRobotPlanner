...
CRITIC_PROMPT_FILE = Path("/home/nx/nx_ws/src/vlm/vlm/prompts/critic_prompt.txt")
...
# def _execute_pseudocode_lines(lines: list[str]) -> None:                            # pseudo_function을 순서대로 수행 및 수행 완료 출력
#     for ln in lines:
...
def run(                                                                            # 전체 run 함수
    ...
    for loop_idx, slam_json in enumerate(slam_stream, start=1):
        ...
        #     for ln in executed:
        #         _execute_pseudocode_lines([ln])              # 한 줄씩 실행
        #         update_inventory_from_lines([ln], world, inventory)     # 한 pseudo_function 실행 후 inventory update
        #     history_blocks.append("\n".join(pseudo_lines))            # 히스토리에 '이번 루프 전체 계획'을 문자열로 저장
        #     print("[RUN] search()까지 실행 후 다음 루프로 재계획")
        #     # 다음 루프에서 slam_stream으로 들어오는 최신 JSON을 받아 계속 진행
        #     continue
...
class DetectedObjectsNode(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter("keywords_topic", "keyworlds")
        ...
        self.goal_pose_topic: str = self.get_parameter("goal_pose_topic").get_parameter_value().string_value
    ...
    def publish_goal(self, x: float, y: float, yaw: float = 0.0):
        ...
        self.get_logger().info(
            f"goal_pose -> '{self.goal_pose_topic}': x={x:.3f}, y={y:.3f}, yaw={yaw:.3f}"
        )
